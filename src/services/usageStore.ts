...
export interface UsageSummary {
  ...
  byComplexityRoute: Record<string, { requests: number; cost: number }>;
...
export class InMemoryUsageStore implements UsageStore {
  ...
  summary(query: UsageQuery = {}): UsageSummary {
    ...
    const byComplexityRoute: UsageSummary["byComplexityRoute"] = {};
    let totalCost = 0;
    let totalTokensIn = 0;
    ...
    for (const r of rows) {
      ...
      const complexity = (byComplexityRoute[r.complexityRoute] ??= { requests: 0, cost: 0 });
      complexity.requests++;
      complexity.cost = round6(complexity.cost + r.cost);
    }
