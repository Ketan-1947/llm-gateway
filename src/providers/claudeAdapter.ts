...
export class ClaudeAdapter implements ProviderAdapter {
  ...
  private requireClient(): Anthropic {
    if (!this.client) {
      throw new GatewayError(
        502,
        "provider_unavailable",
        "Anthropic is not configured (ANTHROPIC_API_KEY is empty).",
      );
    }
    return this.client;
  }
  ...
  async call(req: LLMRequest): Promise<LLMResponse> {
    ...
    try {
      const msg = await client.messages.create(buildParams(req));
  ...
  async *stream(req: LLMRequest): AsyncGenerator<string, LLMResponse, void> {
    ...
    try {
      ...
      stream = client.messages.stream(buildParams(req));
    } catch (err) {
      throw wrapError(err, Date.now() - start);
    }
